...
const ProductDetail=(props,ref)=>{
    ...
    const formik = useFormik({
        ...
        initialValues: props.data ? {
            ...
            detailDescription: props.data.detailDescription,
        ...
        validate: (data)=>{
            ...
            if (data.description.length > 2000) errors.description = 'Tối đa 2000 chữ cái';
        ...
        onSubmit: async (data)=>{
            global.loading.show(); 
            let submitAlbum = [];
            data.album.forEach(i=>(i.name)?submitAlbum.push(i.name):submitAlbum.push(i));   
            let updateData={
                _id: data._id,
                name: data.name,
                ...
                price: data.price,
                album: submitAlbum,
                promotion: data.promotion ? JSON.stringify(data.promotion) : undefined,
                blog: data.blog
            }
            if(data.album) updateData={...updateData,albumFile:data.album}                      
                const res = await productService.update(updateData);
                switch (res.status){
                    case statusCode.OK:
                        const {product} = res.data.payload;
                        global.toast.show({severity:'success', summary: 'Cập nhật sản phẩm', detail: 'Thành công', life: 1500});
                        let initialValues = {};
    ...
    const addDescriptionDetailFieldInput = () => formik.setFieldValue('detailDescription',[...formik.values.detailDescription, {title:'',content: ''}]);
            ...
            <form className="product-detail p-grid p-justify-center" onSubmit={formik.handleSubmit}>
                <div className='p-col-8 p-d-flex p-jc-center'>
                    <div className='p-col-6 p-pt-6'>
                        <OrderImageList id = "album" allowRemove items={formik.values.album} 
                            onRemove={(item)=>formik.setFieldValue('album',formik.values.album.filter(f=>f!==item))}  
                            onChange={(value)=>formik.setFieldValue('album',value)}/>  
                    </div>
                    <div className='p-col-5 p-pt-6'>
                        <FileUpload 
                            ...
                            onClear={()=>formik.setFieldValue('album',formik.values.album.filter(i=>i.objectURL===undefined),true)}
                            multiple={true}
                            isValidFileHandler={(isValid)=> formik.setFieldError('album',isValid ? '' : 'File không hợp lệ')}
                ...
                <div className='p-col-6 p-fluid'>
                    ...
                    <div className='p-mt-5'>
                        ...
                        <div className='p-d-flex p-jc-between p-mt-5'>
                            <span className="p-float-label">
                                <InputNumber id="promotionValue" name="promotionValue"  min={0} max={100}
                                    value={formik.values.promotion ? formik.values.promotion.value : undefined}
                                    onValueChange={e=> formik.setFieldValue('promotion',{...formik.values.promotion, value: e.value})}/>
                                <label htmlFor="promotionValue">Giá trị {`(%)`}</label>
                            </span>
                            <span className="p-float-label">
                                <Calendar id="promotionStartDate" name="promotionStartDate" showIcon
                                    value={formik.values.promotion ? formik.values.promotion.startDate : ''}
                                    onChange={e=> formik.setFieldValue('promotion',{...formik.values.promotion, startDate: e.value})}/>
                                <label htmlFor="promotionStartDate">Ngày bắt đầu</label>
                            </span>
                            <span className="p-float-label">
                                <Calendar id="promotionEndDate" name="promotionEndDate" showIcon
                                    value={formik.values.promotion ? formik.values.promotion.endDate : ''}
                                    onChange={e=> formik.setFieldValue('promotion',{...formik.values.promotion, endDate: e.value})}/>
